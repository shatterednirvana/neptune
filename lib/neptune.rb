...
# A regular expression that is used to indicate that a given file has been
# randomly generated by neptune (and thus not something the user gave us).
TEMP_FILE_REGEX = /temp-[\w]+\Z/
...
module NeptuneHelper
  ...
  def self.preprocess_babel(job_data, controller)
    ...
    ["@output", "@error", "@metadata"].each { |param|
      if job_data[param] !~ TEMP_FILE_REGEX
        self.require_file_to_not_exist(job_data[param], job_data, controller)
      end
    }
